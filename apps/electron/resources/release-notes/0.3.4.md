...
Place your skills in `~/.agents/skills/` and they'll work with DataPilot, Codex, Gemini, and other tools that support this convention. Your existing workspace skills continue to work as before.
...
When you set `colorTheme` in `~/.datapilot/config.json`, it now properly takes priority over any cached theme in localStorage. This makes it possible for agents to modify the theme via documented config paths.
