...
This guide explains how to customize the visual theme of DataPilot.

## Overview

DataPilot uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.datapilot/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.datapilot/theme.json` - Override specific colors (app-level)
...
```
~/.datapilot/workspaces/{id}/config.json
```
...
Create `~/.datapilot/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.datapilot/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.datapilot/themes/{name}.json`
...
1. Create `~/.datapilot/theme.json` for overrides or `~/.datapilot/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.datapilot/theme.json` for overrides, `~/.datapilot/themes/` for presets)
