...
This guide explains how to create and configure skills in DataPilot.

> **CLI-first workflow (recommended):** Use `dtpilot skill ...` commands instead of editing `SKILL.md` files directly.
> - `dtpilot skill --help`
...
DataPilot uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in DataPilot
...
1. **Workspace skill checked first** - If `~/.datapilot/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.datapilot/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.datapilot/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.datapilot/workspaces/{ws}/skills/commit/SKILL.md`
