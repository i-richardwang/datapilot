...
const PI_PRESETS: Preset[] = [
  { key: 'pi', label: 'DataPilot Backend (Direct)', url: '' },
...
export function ApiKeyInput({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isCustomEndpoint = (activePreset === 'custom' || OPENAI_COMPAT_CUSTOM_URL_PRESETS?.has(activePreset)) && !!effectiveBaseUrl
    const customEndpoint = isCustomEndpoint ? { api: OPENAI_COMPAT_CUSTOM_URL_PRESETS?.has(activePreset) ? 'openai-completions' as const : customApi } : undefined
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              disabled={isDisabled}
              className="flex h-6 items-center gap-1 rounded-md bg-background shadow-minimal pl-2.5 pr-2 text-[12px] font-medium text-foreground/50 hover:bg-foreground/5 hover:text-foreground focus:outline-none"
            >
            ...
            <>
              ...
              {activeTierConfig && tierDropdownPosition && (
                <>
                  ...
                  />
                  <div
                    className="fixed z-floating-menu min-w-[200px] overflow-hidden rounded-lg bg-background text-foreground shadow-modal-small"
                    style={{
                        ...
                        {piModels
                          ...
                          .map((model) => (
                            <CommandPrimitive.Item
                              ...
                              className={cn(
                                "flex cursor-pointer select-none items-center justify-between gap-3 rounded-md px-3 py-2 text-[13px]",
