/**
 * Envelope codec for the DataPilot RPC WebSocket transport.
 *
 * Vendored from `@craft-agent/server-core/src/transport/codec.ts` so the CLI
 * can be published without workspace deps. Keep this in sync with the server's
 * codec; both sides must agree on the wire format for Uint8Array values.
 */
...
function decodeWireValue(value: unknown): unknown {
  ...
  if (isRecord(value)) {
    if (
      ...
      return base64ToBytes(value[WIRE_BASE64_KEY] as string)
    }
...
function validateEnvelopeShape(value: unknown): value is MessageEnvelope {
