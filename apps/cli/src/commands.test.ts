...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    ...
    const prevCa = process.env.DATAPILOT_TLS_CA

    process.env.DATAPILOT_SERVER_URL = 'ws://env-server:8080'
    process.env.DATAPILOT_SERVER_TOKEN = 'env-token'
    process.env.DATAPILOT_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevCa === undefined) delete process.env.DATAPILOT_TLS_CA
      else process.env.DATAPILOT_TLS_CA = prevCa
    }
  })
