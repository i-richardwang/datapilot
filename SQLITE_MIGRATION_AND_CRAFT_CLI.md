...
| v0.8.3 | 2026-04-03 | 0 | **Clean merge — no conflicts.** Session self-management tools (`set_session_labels`, `set_session_status`, `get_session_info`, `list_sessions`), compact mode UI, legacy provider cleanup, Docker build perf. New session tools use `SessionToolContext` callbacks (not direct storage imports), so they work transparently with both JSON and SQLite storage backends. `persistence-queue.ts` auto-merged cleanly (upstream added idle-session metadata protection; our SQLite session storage is unaffected as it uses a separate persistence path). No upstream changes to SQLite-migrated storage modules. |
