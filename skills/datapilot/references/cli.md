---
name: datapilot-cli
description: datapilot CLI 命令参考。执行任何 datapilot 命令前加载。只记录 `datapilot <entity>`（不带 action）无法显示的内容：输入规则、字段、返回结构、枚举值、易错点。
---

# CLI 命令参考

**一手信息来源**：`datapilot <entity>` 不带 action 会返回该实体的 action 列表；每个 action 的 positional/flag 用法在出错时通常由 `suggestion` 字段给出。本文件记录 **action 列表无法显示的内容**：输入规则、必需字段、返回 JSON 结构、枚举值、易错点。
...
- **Positional vs flag**：ID / slug 是 positional；身份标识 / 子类型 / 查询参数是普通 flag；业务数据通过 `--input` JSON 传入。

## 探索未知字段的方法

Server 端 schema 可能会演进，硬编码字段容易过时。不确定怎么填 `--input` 时按以下三种方法依次尝试：
...
当前 CLI 暴露 7 个实体：`session` / `batch` / `automation` / `workspace` / `source` / `skill` / `label`。每个实体完整 action 列表用 `datapilot <entity>` 拿。下面只记录**语义要点、必需字段、返回结构、枚举值、易错点**。

### session — 会话与消息（核心）

**`session create`** — 创建不发消息。**普通 flag 只有 `--name`**；其它字段（`permissionMode`、`enabledSourceSlugs`、`model`、`thinkingLevel`、`isFlagged`……）全部走 `--input '<json>'`。不传 `permissionMode` 时默认 `allow-all`。返回完整 session 对象。
...
- `enabledSourceSlugs` — `string[]`，允许使用的数据源 slug；**不确定 slug 时先通过 `datapilot source list` 查看 `.data[].slug`**。不传则为空集（纯 LLM 无外部工具）
...
`source.path` 和 `output.path` 都指向 server 文件系统（绝对路径或相对 workspace root）。创建新 batch 前输入数据需要已在 server 路径上 —— 常见来源：已有 source、之前 session 写入的产物、共享挂载，以及 `workspace get` 返回 `connection.sameMachine = true` 时本地路径也可用（CLI 与 server 同机）。**远程 server 时 CLI 没有文件上传通道**：要么调用方自行将文件放到 server 可达路径，要么先起一个 session 让 agent 准备好输入再 `batch create`。

`batch items` 返回的是每条 item 的 state（status / sessionId / summary / error），不含 `output.jsonl` 的原文 —— 要原文得另起 session 去读那个文件。

**`batch create`** — **普通 flag 只有 `--name`**；其它字段走 `--input`。`--name` 也可以放 `--input` 里。返回 `{id, ...}`。

**`--input` 顶层字段**（schema 可能演进，不确定时参考「探索未知字段的方法」中的方法 2，查看现有 batch 结构）：
...
**`automation create --name <n> --event <EventName> [--input '<json>']`** — `--name` 和 `--event` 是 普通 flag，其它 matcher 字段走 `--input`。**合法事件名不用硬记**：不带 `--event` 直接跑，错误消息里会列出所有合法值。
...
**`automation list`** — 按事件分组返回：`{automations: {EventName: [matcher, ...]}}`。查看某个事件的 matcher 字段结构：`jq '.data.automations.<EventName>[0]'`。

**`automation history <id> [--limit 50]`** — **id 必须作为位置参数**；返回该 automation 的历史执行条目。

**`automation test --input '<json>'`** — 试运行一次，验证 action 是否可执行。输入至少需要 `{event, actions}`；`workspaceId` 由 CLI 自动补充。prompt 类型的 action 会**实际创建 session**（返回 `sessionId`），不经过 matcher 过滤（不做事件匹配，直接执行 actions）。示例：
...
**`enable` 之后 automation 在 server 端独立运行** —— CLI 只是配置接口，enable 后由 server 常驻执行。
...
写操作（create/update/delete）主要用于管理 DataPilot 自身配置，agent-to-agent 场景较少使用；**读操作**（list/get）常用于查询 slug 和数据结构。

### source — 数据源（MCP / REST API / 本地 FS）

- **`source list`** 返回扁平数组。为 session 设置 `enabledSourceSlugs` 前先在此查看 `.data[].slug`
- `source get <slug>` 返回 `{...source, permissions, mcpTools}`
- `source create` 的 普通 flag：`--name` + `--provider` + `--type`；其它字段走 `--input`
...
- `skill create` 的 普通 flag **只有 `--name`**；**slug 由 server 从 name 自动推导**，要覆盖就在 `--input` 里传 `slug`
...
- `label create` 的 普通 flag 只有 `--name`；`color` / `parentId` / `valueType` 走 `--input`
- `label update <id>` 没有 普通 flag，全部走 `--input`；`valueType: "none"` 会清空
