...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if pgrep -x "DataPilot" >/dev/null 2>&1; then
        ...
        if pgrep -x "DataPilot" >/dev/null 2>&1; then
            ...
            pkill -9 -x "DataPilot" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  DataPilot has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'DataPilot'${NC}"
    echo ""
