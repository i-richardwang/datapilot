...
- **storage.db.ts:** Session CRUD against workspace.db (active implementation). Four write tiers — use the narrowest that fits:
  ...
  - `saveSessionTurnReconcile(session, recentlyChangedMessageIds)` — the per-turn reconciliation anchor (SessionManager.onProcessingStopped via persistSessionTurnEnd). Verifies DB rows are an exact positional prefix of memory with an id+position-only select, then upserts only this turn's rows + new tail rows; ANY drift (removal/reorder/gap) falls back to the full saveSession rewrite. SessionManager feeds it `turnChangedMessageIds` (ids tracked through persistSessionMessages since the last full/reconcile persist). **Discipline invariant:** any SessionManager code path that edits an EXISTING message in place must call `persistSessionMessages` with that id (new tail pushes are auto-detected, in-place edits are not) — an unsynced in-place edit passes the prefix check and silently never reaches disk.
  - `saveSession(session)` — full rewrite (delete-all + reinsert-all messages, O(history) sync write). Required for any path that REMOVES messages (branch, queue-cancel, compaction).
- **Hot-path invariant:** every save above ends with `dbEvents.emit('session:saved')`, and the ConfigWatcher's DB-mode listener runs synchronously inside that emit — on the streaming path this fires per persisted message. The listener must stay a single sessions-row select (`loadSessionHeader`); calling full `loadSession()` there re-reads the entire history per message persist (O(history), event-loop blocking — the exact regression fixed in 2026-06).
...
Processing/persistence order is `BatchState.itemOrder` (mirrors `batch_items.position`; rebuilt from row order on every load, never persisted in the meta blob). **Never derive order from `Object.keys/entries(state.items)`** — item ids come from the data source's `idField` and are often integer-like strings, which JS objects iterate in ascending numeric order regardless of insertion order. Dispatch (`dispatchNext`), full saves (`replaceItemRows`), and the paged read path (`getBatchItemsPage`) must go through `itemOrder`; reordering `batch_items.position` externally (e.g. shuffling pending items) is a supported operation and must survive load → full save round-trips. The read path also means the renderer must carry items as an ordered array end-to-end — never round-trip a page slice through a `Record`/`Object.fromEntries`, which re-applies the same integer-key reordering at the UI layer.

### Batch tool profile (`BatchExecution.toolProfile`: `default` | `minimal`)
A batch's `toolProfile` must trim three tool channels **plus** the system prompt identically on both the Claude and Pi backends — they take different code paths, so a change to one needs the mirror applied to the other (keep them in sync, there is no shared enforcement):
- **Built-in/SDK tools** — Claude: `minimal` → `MINIMAL_BATCH_TOOLS` (`['WebSearch','WebFetch','Read','Bash']`) vs `claude_code` preset. Pi: the subprocess (`pi-agent-server` `ensureSession`) narrows `builtinDefs` to `read`+`bash` (web tools `web_search`/`web_fetch` always kept) when `InitMessage.toolProfile === 'minimal'`. Pi tool names are lowercase/`web_*`, so the set is hand-mirrored, not the Claude constant.
- **Session proxy tools** — both backends route through `getSessionToolDefs`/`getToolDefsAsJsonSchema`; pass `batchMode` (default → `BATCH_EXCLUDED_TOOLS` blacklist) and `minimalBatchMode` (→ `MINIMAL_BATCH_SESSION_TOOLS` whitelist = `batch_output` only).
- **Source/pool MCP tools** — Claude filters `mcpServers` to `MINI_AGENT_MCP_KEYS=['session']` for `minimal`; Pi gates `registerPoolToolsWithSubprocess()` (covers both the initial registration and mid-session source changes).
- **System prompt** — pass `batchMode` to `getSystemPrompt`; `isBatch` drops UI/CLI/sources sections, `isMinimalBatch` drops more. Pi reads the per-session `BatchContext` via `getSessionBatchContext(this._sessionId)` (the canonical id used everywhere in the agent — do not re-derive a session id locally for batch lookups).
...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.datapilot/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
