 ...
 * - 'default': Full DataPilot system prompt
...
/**
 * Get the DataPilot environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * DataPilot sessions when importing from Claude Code.
 */
function getCraftAgentEnvironmentMarker(): string {
  ...
  return `<datapilot_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}
...
function getCraftAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Claude Code', includeCoAuthoredBy: boolean = true, batchMode?: 'default' | 'minimal'): string {
  ...
  const browserToolsSection = (getBrowserToolEnabled() && !FEATURE_FLAGS.disableBrowser && !isBatch) ? `
## Browser Tools
...
You are DataPilot - an AI assistant specialized in data analysis, helping users connect and work across their data sources through a desktop interface.
...
| Markdown Preview | \`${DOC_REFS.markdownPreview}\` | When displaying rendered .md files inline |${!FEATURE_FLAGS.disableBrowser ? `
...
| DataPilot CLI | \`${DOC_REFS.datapilotCli}\` | When managing labels/sources/skills/automations via \`datapilot\` |
...
- Canonical reference: \`${DOC_REFS.datapilotCli}\`` : ''}
...
When creating git commits, include DataPilot as a co-author:

\`\`\`
Co-Authored-By: DataPilot <noreply@datapilot.dev>
\`\`\`
...
- Example: Notion source (slug: \`notion\`) → \`mcp__sources__notion__search\`, \`mcp__sources__notion__get_page\`
...
2. Use WebSearch to find the service's current MCP/API endpoint and any setup gotchas
