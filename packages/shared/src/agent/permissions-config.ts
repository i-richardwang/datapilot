 ...
 *
 * File locations:
 * - Workspace: ~/.datapilot/workspaces/{slug}/permissions.json
 * - Per-source: ~/.datapilot/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.datapilot/permissions/
 * Reads env var dynamically so tests can override via DATAPILOT_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.DATAPILOT_CONFIG_DIR || join(homedir(), '.datapilot');
...
/**
 * Load default permissions from ~/.datapilot/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.batchCli && pattern.startsWith('^craft-agent-batch\\s')) {
    return false;
  }
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.datapilot/permissions/default.json)
   ...
   * These come from ~/.datapilot/permissions/default.json
   */
