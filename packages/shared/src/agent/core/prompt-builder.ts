...
export class PromptBuilder {
  ...
  buildContextParts(
    ...
    return [
      ...
      ...this.buildStableContextParts(options),
    ];
  }
   ...
   *  3. batch output instructions, for batch sessions with an output schema
   *     (fork-only; session-fixed, so it belongs in the cached prefix)
   *
   * Pure and idempotent: holds no one-shot state (batchOutputSchema is fixed
   * for the session), so it is safe to call any number of times per turn.
   *
   * @param options - Context building options (only batchOutputSchema is read)
   */
  buildStableContextParts(options?: ContextBlockOptions): string[] {
    const parts: string[] = [];
