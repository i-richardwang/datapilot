...
// Documentation is served via local files at ~/.datapilot/docs/
...
export class ClaudeAgent extends BaseAgent {
   ...
   * `DATAPILOT_KEEP_BG_AGENTS_ALIVE` via the shared resolver. Default is ON (opt-out);
   * set `DATAPILOT_KEEP_BG_AGENTS_ALIVE=0` to force the per-turn kill-switch path.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const fullMcpServers: Options['mcpServers'] = {
        ...
        // backend. Local apps/electron/resources/docs/sources.md covers the schema; agent falls
        // back to WebSearch for service-specific URLs/auth. Re-enable by uncommenting.
        // 'craft-agents-docs': {
        //   type: 'http',
      ...
      // Gated by enable1MContext in global config (~/.datapilot/config.json).
