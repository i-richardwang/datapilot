...
function readClipboardMacOS(): FileAttachment[] {
  ...
  try {
    const scriptFile = join(tmpdir(), `datapilot-clipboard-files-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
...
function readClipboardImageDataWindows(): FileAttachment | null {
  const tempFile = join(tmpdir(), `datapilot-clipboard-${Date.now()}.png`);
...
function readClipboardImageDataMacOS(): FileAttachment | null {
  ...
  try {
    const scriptFile = join(tmpdir(), `datapilot-clipboard-script-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
ObjC.import('Foundation');
