...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for DataPilot',
  defaults: {
...
/**
 * Load config defaults from ~/.datapilot/config-defaults.json
...
export function ensureConfigDir(): void {
  ...
  // Initialize bundled docs (creates ~/.datapilot/docs/ with sources.md, agents.md, permissions.md)
  initializeDocs();
...
/**
 * Get the path to the app-level theme override file (~/.datapilot/theme.json).
 */
 ...
 * Preset themes are stored at ~/.datapilot/themes/
 */
...
function migrateLegacyProviderTypes(config: StoredConfig): boolean {
  ...
  for (const connection of config.llmConnections) {
    ...
    // anthropic_compat is a live provider type in our fork (routes through ClaudeAgent SDK)
    // — do NOT migrate it to pi_compat.
...
/**
 * Returns the path to the tool-icons directory: ~/.datapilot/tool-icons/
 */
