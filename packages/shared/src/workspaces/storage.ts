 ...
 * Default location: ~/.datapilot/workspaces/
 */
...
/**
 * Get the default workspaces directory (~/.datapilot/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.datapilot/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.datapilot/workspaces/
 */
 ...
 * Resolution order: ENV (DATAPILOT_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.DATAPILOT_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `datapilot-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
