...
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, cleanupSessionScopedTools, mergeSessionScopedToolCallbacks, registerSessionBatchContext, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary, resolveKeepBackgroundTasksAlive } from '@craft-agent/shared/agent'
import {
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.datapilot/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.datapilot/tool-icons/
...
function managedMatchesPageFilter(m: ManagedSession, f: SessionListPageFilter, viewPredicate?: SessionViewPredicate<ManagedSession>): boolean {
  ...
  if (f.projectInclude && f.projectInclude.length > 0) {
    if (!m.projectId || !f.projectInclude.includes(m.projectId)) return false
  }
  if (f.projectExclude && f.projectExclude.length > 0) {
    if (m.projectId && f.projectExclude.includes(m.projectId)) return false
  }
...
export class SessionManager implements ISessionManager {
  ...
  /**
   * WS2 keep-alive flag (default ON, opt-out via `DATAPILOT_KEEP_BG_AGENTS_ALIVE=0`).
  ...
  async createSession(
    ...
    if (validatedBranch) {
      ...
      for (const sub of ASSET_SUBDIRS) {
        const src = join(sourceDir, sub)
        const dst = join(branchDir, sub)
  ...
  private isHibernationSafe(managed: ManagedSession): boolean {
    ...
    // Keep-alive background sub-agents ride the live agent's persistent query;
    // hibernation disposes the agent and would kill them mid-run. (Without
    // keep-alive they are flipped to 'orphaned' at turn end, so this is a no-op.)
