 ...
 *
 * Environment:
 *   DATAPILOT_SERVER_TOKEN         — required bearer token for client auth
 *   DATAPILOT_RPC_HOST             — bind address (default: 127.0.0.1)
 *   DATAPILOT_RPC_PORT             — bind port (default: 9100)
 ...
 *   DATAPILOT_MESSAGING_WA_WORKER  — absolute path to worker.cjs (default: packages/messaging-whatsapp-worker/dist/worker.cjs)
 *   DATAPILOT_MESSAGING_NODE_BIN   — Node binary used to spawn the WhatsApp worker (default: node)
 */
...
if (process.env.DATAPILOT_DEBUG === 'true' || process.env.DATAPILOT_DEBUG === '1') {
  enableDebug()
}
...
const tlsCertPath = process.env.DATAPILOT_RPC_TLS_CERT
const tlsKeyPath = process.env.DATAPILOT_RPC_TLS_KEY
if (tlsCertPath || tlsKeyPath) {
  if (!tlsCertPath || !tlsKeyPath) {
    console.error('TLS requires both DATAPILOT_RPC_TLS_CERT and DATAPILOT_RPC_TLS_KEY.')
    process.exit(1)
  }
  tls = {
    cert: readFileSync(tlsCertPath),
    key: readFileSync(tlsKeyPath),
    ...(process.env.DATAPILOT_RPC_TLS_CA ? { ca: readFileSync(process.env.DATAPILOT_RPC_TLS_CA) } : {}),
  }
}
...
if (webuiEnabled && serverToken) {
  const rpcPort = parseInt(process.env.DATAPILOT_RPC_PORT ?? '9100', 10)
  const rpcProtocol = tls ? 'wss' as const : 'ws' as const

  webuiHandler = createWebuiHandler({
    webuiDir: webuiDir!,
    secret: serverToken,
    password: process.env.DATAPILOT_WEBUI_PASSWORD || undefined,
...
const waWorkerEntry = process.env.DATAPILOT_MESSAGING_WA_WORKER
  ?? join(bundledAssetsRoot, 'packages', 'messaging-whatsapp-worker', 'dist', 'worker.cjs')
const waNodeBin = process.env.DATAPILOT_MESSAGING_NODE_BIN ?? 'node'
...
const healthPort = parseInt(process.env.DATAPILOT_HEALTH_PORT ?? '0', 10)
...
if (!isLocalBind && instance.protocol === 'ws') {
  if (process.argv.includes('--allow-insecure-bind')) {
    console.warn(
      ...
      '   Set DATAPILOT_RPC_TLS_CERT and DATAPILOT_RPC_TLS_KEY to enable wss://.\n'
    )
  } else {
    console.error(
      ...
      '   Options:\n' +
      '     1. Set DATAPILOT_RPC_TLS_CERT and DATAPILOT_RPC_TLS_KEY to enable wss://\n' +
