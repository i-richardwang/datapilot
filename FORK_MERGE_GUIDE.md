...
>
> **Last updated after:** v0.8.0 merge (hybrid transport, remote workspaces, WebUI, session export/import, mobile WebUI)
...
| v0.8.0 | 2026-03-26 | 10 | Hybrid local/remote transport, multiple remote workspaces, browser-accessible WebUI, session export/import, mobile WebUI, supply chain security hardening. Resolved 10 conflicts: 7 border-radius token conflicts (upstream added CSS class prefixes like `header-icon-btn`, `panel-header-btn`, `entity-row-btn`, `input-toolbar-btn` while keeping `rounded-[Npx]` — merged upstream prefixes with our standard Tailwind rounded classes); `feature-flags.ts` — kept fork's `liteVersion` alongside upstream's new `embeddedServer` getter; `AppShell.tsx` — kept fork's `BatchFilter` import + batch delete dialog alongside upstream's `sendToWorkspaceAtom` + `SendToWorkspaceDialog`; `SessionManager.ts` — kept fork's `registerSessionBatchContext` import alongside upstream's `generateConversationSummary`. All fork code verified intact: batch lifecycle, lite-mode filtering, custom endpoint fixes, border-radius tokens. No new `rounded-[Npx]` patterns introduced by upstream. |
